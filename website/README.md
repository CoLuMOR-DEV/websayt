...
Vercel specifically, note three things:

- The bot won't run there (above).
- Vercel's serverless functions have a **read-only filesystem** except
  `/tmp` — so admin edits to the JSON files won't reliably persist
  unless you also set `KV_REST_API_URL`/`KV_REST_API_TOKEN` (a free
  Upstash Redis database via Vercel's Storage tab). Not needed for
  local use.
- **`SESSION_SECRET` is required**, not optional, on Vercel. Set it in
  the project's Environment Variables and redeploy. Without it, the
  admin login *looks* broken — the password is accepted, but you get
  bounced straight back to the login screen (or writes start failing
  with "Login required") — because each serverless instance would
  otherwise sign sessions with its own random secret, and the next
  request can land on a different instance that can't verify a cookie
  signed by another one. Generate one with:
  `node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"`

## 5. Design notes

Dark sigilism theme — near-black background, a `js/cyber-bg.js` canvas
drawing monochrome thorned sigils (long radiating spikes, a dense
cluster of jagged thorns, faint rings, tiny drifting flecks, and a
wavering thorned vine hanging from each node) that slowly rotate and
sway behind everything, plus a faint particle mesh underneath (it
turns static if the browser prefers reduced motion). The schedule grid
switches to a

...
3. Click **Register slash commands** once, then **Start bot**. (The bot
   also re-registers its commands automatically every time it logs in,
   so this recovers on its own if you ever forget the manual step.)

**When inviting the bot to your server**, make sure you check both the
`bot` and `applications.commands` scopes in the Discord Developer
Portal's invite-link generator — if `applications.commands` is missing,
slash commands silently never show up no matter how many times you
register them.

Three schedule-aware features you can toggle right there:
- **Daily schedule post** — posts today's periods to the announcement
  channel at an hour you choose.
- **Class-start reminders** — posts a reminder 30, 20, and 5 minutes
  before each class starts, then deletes those messages an hour after
  the class ends.
...
- `/announce <message>` — posts the message and pings @everyone in the
  channel it's run in. Restricted to members with the Manage Server
  permission, since it's a broadcast ping.
...
a VPS, Railway, Fly.io, etc.) — not on Vercel serverless. If slash
commands seem to "not work" and you're on Vercel, this is almost
always why — the bot can never actually stay online there.
...
Pure black & white "sigilism" theme. `js/cyber-bg.js` draws a light,
slow snowfall behind everything — no color, deliberately cheap to run
(no shadow blur, a capped particle count, and it fully pauses when the
tab isn't visible or the person turns it off with the toggle in the
top-left corner, a choice remembered in `localStorage`). It turns
static if the browser prefers reduced motion. The schedule grid
switches to a day-tabs + agenda list on narrow screens instead of a
horizontally scrolling table. A small music dock (`css/music-widget.css`,
`js/music-widget.js`) tucks itself below the bottom edge of the screen;
moving the mouse near the bottom reveals it, moving away hides it again,
and tapping its handle pins it open on touch devices. Its audio file
uses `preload="none"` so it's never fetched until someone presses play.

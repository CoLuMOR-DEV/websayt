...
let lastDailyPostDate = null; // 'YYYY-MM-DD', so we only post once per day
...
async function postDailySchedule(config) {
  ...
  if (now.getHours() !== Number(config.dailyScheduleHour)) return;
  ...
  try {
    ...
    for (const w of due) {
      const embed = new EmbedBuilder()
        ...
        .setColor(0xff4d6d)
        .setDescription(
          `**${w.courseCode}** — ${w.title}\nDue ${new Date(w.deadline).toLocaleString('en-US', {
...
async function start(configOverride) {
  ...
  if (!GatewayIntentBits) {
    // GatewayIntentBits is a discord.js v14 export. If it's missing, the
    // package actually sitting in node_modules isn't v14 — almost always
    // a stale/partial install. Fail with a clear, actionable message
    // instead of a cryptic "Cannot read properties of undefined" crash.
    status.starting = false;
    status.lastError =
      'Installed discord.js is not v14 (GatewayIntentBits is missing). ' +
...
module.exports = { start, stop, getStatus, deployCommands, autoStartIfConfigured };

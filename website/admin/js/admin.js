(() => {
      ...
      list.innerHTML = entries.map((e) => `
        <li class="admin-item" data-id="${e.id}">
          <div class="admin-item__main">
            ...
            <div class="admin-item__sub">${formatTime12(e.startTime)} – ${formatTime12(e.endTime)} · ${escapeHtml(e.classType || 'Face-to-Face')}${e.room ? ' · ' + escapeHtml(e.room) : ''}${e.meetLink ? ' · has Meet link' : ''}</div>
          </div>
  ...
  function editSchedule(entry) {
    ...
    document.getElementById('classType').value = entry.classType || 'Face-to-Face';
    document.getElementById('instructor').value = entry.instructor || '';
  ...
  async function loadBotConfig() {
    try {
      ...
      document.getElementById('botClassRemindersEnabled').checked = !!cfg.classReminderEnabled;
  ...
  document.getElementById('botFeaturesSaveBtn').addEventListener('click', async () => {
    const payload = {
      ...
      classReminderEnabled: document.getElementById('botClassRemindersEnabled').checked,

...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Most commonly: SESSION_SECRET isn't set on Vercel. The password was
    // right, but we can't safely issue a session — surface that clearly
    // instead of returning ok:true and having the panel mysteriously log
    // the person back out on the next request.
    console.error('Session creation error:', err.message);
    return res.status(500).json({ error: 'Server session is not configured. See .env.example (SESSION_SECRET).' });
  }
};

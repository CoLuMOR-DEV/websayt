...
async function postDailySchedule(config) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      entries.length
        ? entries.map((e) => {
            const typeTag = e.classType === 'Online Class' ? 'Online' : 'Face-to-Face';
            const roomTag = e.room ? ` · ${e.room}` : '';
            return `**${e.courseCode}** — ${e.courseName}\n${formatTime12(e.startTime)} – ${formatTime12(e.endTime)} · ${typeTag}${roomTag}${e.meetLink ? ` · [Join](${e.meetLink})` : ''}`;
          }).join('\n\n')
    ...
    if (remainingToStart >= 0) {
      for (const lead of CLASS_REMINDER_LEADS_MIN) {
        if (remainingToStart <= lead && !state.sent.has(lead)) {
          ...
          try {
            const channel = await client.channels.fetch(config.announceChannelId);
            const typeTag = entry.classType === 'Online Class' ? 'Online Class' : 'Face-to-Face';
            const roomTag = entry.room ? ` · ${entry.room}` : '';
            const embed = new EmbedBuilder()
              .setTitle(`${entry.courseCode} starts in ${lead} minute${lead === 1 ? '' : 's'}`)
              .setColor(0xE5E5E5)
              .setDescription(
                `**${entry.courseCode}** — ${entry.courseName}\n${formatTime12(entry.startTime)} – ${formatTime12(entry.endTime)} · ${typeTag}${roomTag}` +
...
async function start(configOverride) {
  ...
  newClient.once('ready', () => {
    ...
    // Best-effort auto-registration, so a fresh setup doesn't silently
    // sit there with commands Discord has never heard of — this was the
    // most common reason "the slash commands don't work" turned out to
    // be true: the bot was online, but nobody had ever pressed the
    // separate "Register slash commands" button. Guild-scoped commands
    // (if a Server ID is saved) show up instantly; global ones can take
    // up to an hour the first time, same as before.
    deployCommands({ ...readConfig(), clientId: readConfig().clientId || newClient.user.id })
      .then((result) => console.log(`Slash commands auto-registered: ${result.count} (${result.scope})`))
      .catch((err) => console.warn('Slash command auto-registration failed:', err.message));
  });

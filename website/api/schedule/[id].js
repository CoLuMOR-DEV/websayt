...
module.exports = async (req, res) => {
  ...
  if (req.method === 'PUT') {
    ...
    schedule[index] = {
      ...
      instructor: body.instructor !== undefined ? String(body.instructor).trim() : (schedule[index].instructor || ''),
      room: body.room !== undefined ? String(body.room).trim() : (schedule[index].room || ''),
      classType: VALID_CLASS_TYPES.includes(body.classType) ? body.classType : (schedule[index].classType || 'Face-to-Face'),
      meetLink: body.meetLink ? String(body.meetLink).trim() : '',
      color: body.color || schedule[index].color || '#C6C6C6',
    };

// js/music-widget.js
//
// A small always-there music dock tucked at the top of the screen,
// just under the nav. Closed by default — only the handle tab is
// visible poking down from the top edge. It slides down when the
// mouse moves near the top of the viewport (or the handle is tapped
// on touch devices) and slides back up when the mouse moves away.
// Clicking the handle also "pins" it open so it stays down until
// clicked again — this is what makes it usable on touch screens,
// which have no hover to trigger the slide.
//
...
(() => {
  ...
  const EDGE_ZONE = 90; // px from the top of the viewport that triggers a reveal

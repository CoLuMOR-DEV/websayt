  ...
  function renderBlocks(entries) {
    entries.forEach((entry) => {
      ...
      block.innerHTML = `
        ...
        ${(entry.classType || entry.room) ? `<div class="block__meta"><span class="type-pill type-pill--${classTypeSlug(entry.classType)}">${escapeHtml(entry.classType || 'Face-to-Face')}</span>${entry.room ? ` · ${escapeHtml(entry.room)}` : ''}</div>` : ''}
      `;

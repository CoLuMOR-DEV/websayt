(() => {
  ...
  function buildGridShell() {
    ...
    DAYS.forEach((day, colIdx) => {
      ...
      head.className = 'day-head' + (colIdx === 0 ? ' day-head--first' : '') + (day === todayCode ? ' is-today' : '');
    ...
    DAYS.forEach((day, colIdx) => {
      ...
      col.className = 'day-col' + (colIdx === 0 ? ' day-col--first' : '') + (day === todayCode ? ' is-today' : '');
      col.dataset.day = day;
  ...
  function renderBlocks(entries) {
    entries.forEach((entry) => {
      ...
      block.innerHTML = `
        ...
        ${(entry.classType || entry.room) ? `<div class="block__meta"><span class="type-pill type-pill--${classTypeSlug(entry.classType)}">${escapeHtml(entry.classType || 'Face-to-Face')}</span>${entry.room ? `<span class="block__room">${escapeHtml(entry.room)}</span>` : ''}</div>` : ''}
      `;
  ...
  function classTypeSlug(classType) {
    return classType === 'Online Class' ? 'online' : 'f2f';
  }
  ...
  function openModal(entry) {
    ...
    const metaParts = [];
    if (entry.classType) metaParts.push(`<span class="type-pill type-pill--${classTypeSlug(entry.classType)}">${escapeHtml(entry.classType)}</span>`);
    if (entry.room) metaParts.push(escapeHtml(entry.room));
    if (entry.instructor) metaParts.push(escapeHtml(entry.instructor));
    ...
    list.innerHTML = dayEntries.map((entry, i) => `
      <button type="button" class="agenda-item" data-index="${i}" style="border-left-color:${entry.color || 'var(--cyan)'}">
        ...
        <div>
          ...
          ${(entry.classType || entry.room) ? `<div class="agenda-item__meta"><span class="type-pill type-pill--${classTypeSlug(entry.classType)}">${escapeHtml(entry.classType || 'Face-to-Face')}</span>${entry.room ? ` · ${escapeHtml(entry.room)}` : ''}</div>` : ''}
        </div>
      </button>
    `).join('');
